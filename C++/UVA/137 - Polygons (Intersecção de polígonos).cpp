...
void interPolygon(vector<pv> &polygon1, vector<pv>& polygon2, vector<pv> &interseccao){
...
int main(){
	int n, m;
	ld x, y;
	vector<pv> polygon1, polygon2, interseccao;
	while (cin >> n, n)
	{
		polygon1.clear();
		polygon2.clear();
		...
		interPolygon(polygon1, polygon2, interseccao);
		//~ cout << interseccao.size() << endl;
		string aux;
		stringstream ss;
		ss << fixed << setprecision(2) << (area(polygon1) + area(polygon2) - 2*area(interseccao));
		ss >> aux;
		//~ cout << setw(8) << aux << endl;
		cout << setw(8) << aux;
		//~ cout << fixed << setprecision(2) << (area(polygon1) + area(polygon2) - 2*area(interseccao));
