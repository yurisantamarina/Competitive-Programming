...
int visivel(pv pt){
	...
	//~ printf("pt = "); printaPonto(pt); printf("e visivel\n");
	return 1;
...
int sweep(){
	...
	{
		//~ printf("estou em "); printaPonto(eve[i].pt);
		//~ printf(" tipo = %s\n", eve[i].tipo == 0 ? "ABRE" : eve[i].tipo == 1 ? "QUERY" : "FECHA");
		if(eve[i].tipo == ABRE){
			//~ printf("sz antes = %d\n", (int)active.size());
...
int solve(int idx){
	...
	for (int i = 0; i < qtdParede; i++)
	{
		...
		if(!needCut(nw)){
			...
			//~ printf("quebrando "); printaPonto(nw.a); printf("- "); printaPonto(nw.b); printf("\n");
