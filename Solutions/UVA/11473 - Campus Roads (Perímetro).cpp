...
int main(){
	...
	while (tc--)
	{
		vector<pv> road;
		cin >> n >> k;
		//~ printf("%d segmentos para cortar em %d pedaços\n", n, k);
		double per = 0.0;
		for (int i = 0; i < n; i++)
		{
			cin >> x >> y;
			road.pb(pv(x, y));
			if(i) per += distPt(road[i], road[i-1]);
		}
		
		k--;
		double gap = per / (1.0*(k));
		//~ printf("per = %.2Lf\n", per);
		//~ printf("gap = %.2Lf\n", gap);
		cout << "Road #" << caso++ << ":\n";
		cout << fixed << setprecision(2);
		cout << road[0].x << " " << road[0].y << "\n";
		...
		while (botou < k)
		{
			...
			if(d == need){
				...
				//~ printf("Estou em "); printaPonto(at);
				//~ cout << endl;
				v = road[r+1] - road[r];
				v = v / norma(v);
				at.x = at.x + v.x * need;
				at.y = at.y + v.y * need;
				need = gap;
				//~ printf("NOVO Estou em "); printaPonto(at);
				//~ cout << endl;
				cout << at.x << " " << at.y << "\n";
				botou++;
			}else{
