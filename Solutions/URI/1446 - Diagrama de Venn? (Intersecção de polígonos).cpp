...
string func(int x){
	if(x==1) return "CORTOU";
	return "NÃO CORTOU";	
}

vector<pv> polygon[4];
vector<pv> inter123, inter12, inter13, inter23;
bool compare(pair<int, ld> a, pair<int, ld> b){
	if(igual(a.S, b.S)) return a.F < b.F;
	return maior(a.S, b.S);
}
void reset(){
	for (int i = 0; i < 4; i++)
	{
...
int main(){
	...
	while (cin >> n)
	{
		...
		ld areaTot1, areaTot2, areaTot3, areaTot12, areaTot13, areaTot23;
		areaTot1 = area(polygon[1]);
		areaTot2 = area(polygon[2]);
		areaTot3 = area(polygon[3]);
		
		//~ printf("areaTot3 = %.2Lf\n", areaTot3);
		
		ld area1, area2, area3, area12, area13, area23, area123;
		
		interPolygon(polygon[1], polygon[2], inter12);
		areaTot12 = area(inter12);
		
		interPolygon(polygon[1], polygon[3], inter13);
		areaTot13 = area(inter13);
		//~ printf("areaTot13 = %.2Lf\n", areaTot13);
		
		interPolygon(polygon[2], polygon[3], inter23);
		areaTot23 = area(inter23);
		
		interPolygon(inter12, polygon[3], inter123);
		area123 = area(inter123);
		//~ printf("area123 = %.2Lf\n", area123);
		
		area1 = areaTot1 - areaTot12 - areaTot13 + area123;
		area2 = areaTot2 - areaTot12 - areaTot23 + area123;
		area3 = areaTot3 - areaTot13 - areaTot23 + area123;
		area12 = areaTot12 - area123;
		area13 = areaTot13 - area123;
		area23 = areaTot23 - area123;
		
		vector<pair<int, ld> > saida;
		map<int, ld> mapa;
		
		mapa[(cor[1])%16] += area1;
		mapa[(cor[2])%16] += area2;
		mapa[(cor[3])%16] += area3;
		mapa[(cor[1]+cor[2])%16] += area12;
		mapa[(cor[1]+cor[3])%16] += area13;
		mapa[(cor[3]+cor[2])%16] += area23;
		mapa[(cor[1]+cor[2]+cor[3])%16] += area123;
		...
		{
			if(igual(saida[i].S, 0.0)) break;
			cout << saida[i].F << " " << fixed << setprecision(2) << saida[i].S << "\n";
		}
		cout << "\n";
	}
